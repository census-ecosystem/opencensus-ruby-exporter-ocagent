...
describe OpenCensus::Trace::Exporters::OCAgent::Converter do
  let(:service_name) { "ocagent-trace-exporter-service"}
  let(:resource) {
    OpenCensus::Proto::Resource::V1::Resource.new(type: "test")
  }
  let(:converter) {
    OpenCensus::Trace::Exporters::OCAgent::Converter.new(resource)
  }
  ...
  let(:string_truncated_bytes) { 5 }
  let(:truncatable_string) {
    OpenCensus::Trace::TruncatableString.new(
     simple_string,
     truncated_byte_count: string_truncated_bytes
   )
  }
  let(:loc1) {
  ...
  describe "#make_truncatable_string" do
    it "create with default value" do
      proto = converter.make_truncatable_string "test"
      proto.value.must_equal "test"
      proto.truncated_byte_count.must_equal 0
    end

    it "create with byte count" do
      proto = converter.make_truncatable_string "test", 5
      proto.value.must_equal "test"
      proto.truncated_byte_count.must_equal 5
    end
  end

  describe "#convert_truncatable_string" do
    it "converts a truncatable string" do
  ...
  describe "#convert_time" do
    it "converts to protobuf timestamp object" do
      time = Time.now
      proto = converter.convert_time time
      proto.seconds.must_equal time.to_i
      proto.nanos.must_equal time.nsec
    end
  end
  ...
  describe "#convert_attribute_value" do
    ...
    it "converts an double" do
      proto = converter.convert_attribute_value 10.1
      proto.double_value.must_equal 10.1
      proto.value.must_equal :double_value
    end
  ...
  describe "#convert_attributes" do
    it "converts attributes of different types" do
      input_attrs = {
        "str" => truncatable_string,
        "int" => 100,
        "double" => 51.1,
        "bool" => false
      }
      ...
      proto.attribute_map["int"].int_value.must_equal(100)
      proto.attribute_map["int"].value.must_equal :int_value

      proto.attribute_map["double"].double_value.must_equal(51.1)
      proto.attribute_map["double"].value.must_equal :double_value
  ...
  describe "#convert_message_event" do
    it "converts a message event" do
      ...
      proto.message_event.uncompressed_size.must_equal 100
      proto.message_event.compressed_size.must_equal 0
  ...
  describe "#convert_status" do
    it "converts a status" do
      proto = converter.convert_status status
      proto.code.must_equal 404
      proto.message.must_equal "Not found"
    end

    it "converts nil" do
      proto = converter.convert_status nil
      proto.must_be_nil
    end
  end
  ...
  describe "#convert_bool" do
    it "converts a boolean" do
      proto = converter.convert_bool true
      proto.value.must_equal true
    end

    it "converts nil" do
      proto = converter.convert_bool nil
      proto.must_be_nil
    end
  end

  describe "#convert_int32" do
    it "converts a integer" do
      proto = converter.convert_int32 1000
      proto.value.must_equal 1000
    end

    it "converts nil" do
      proto = converter.convert_int32 nil
      proto.must_be_nil
    end
  end
  ...
  describe "#convert_span" do
    it "converts a basic span" do
      ...
      proto.trace_id.must_equal trace_id
      proto.span_id.must_equal span_id
      proto.name.value.must_equal simple_string
      proto.name.truncated_byte_count.must_equal string_truncated_bytes
